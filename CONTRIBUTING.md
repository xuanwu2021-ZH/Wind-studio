...
7.  **Promote Usage**: Promote Wind Studio through blogs, social media, and other channels to attract more users and developers.
...
Automated tests are triggered on pull requests (PRs) opened by members of the Wind Studio organization, except for draft PRs. PRs opened by new contributors will initially be marked with the `needs-ok-to-test` label and will not be automatically tested. Once a Wind Studio organization member adds `/ok-to-test` to the PR, the test pipeline will be created.
...
Thank you for your support and contributions! We look forward to working with you to make Wind Studio a better product.
