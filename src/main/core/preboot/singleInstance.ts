...
/**
 * Require this process to be the primary Windbot Studio instance.
 *
 * Claims Electron's single-instance lock via `app.requestSingleInstanceLock()`.
 * If another Windbot Studio process already holds the lock, this function
...
export function requireSingleInstance(): void {
  ...
  logger.info('Another Windbot Studio instance already holds the single-instance lock; exiting')
  application.quit()
  process.exit(0)
}
