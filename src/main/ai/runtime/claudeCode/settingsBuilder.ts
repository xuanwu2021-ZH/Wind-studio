/**
 * Builds ClaudeCodeSettings from Windbot Studio's agent session configuration.
 *
 * Maps Windbot Studio's internal data model (agent sessions, providers, MCP servers,
...
const MINIMAL_CHERRY_ASSISTANT_INSTRUCTIONS =
  'Within Windbot Studio, serve as Cherry Assistant, its built-in general-purpose Agent and onboarding guide. Help the user complete any request using the available tools.'
...
/**
 * Build session-level ClaudeCodeSettings from Windbot Studio's agent session.
 */
...
export async function buildSkillWhitelist(agentId: string, cwd: string, builtinRole?: string): Promise<string[]> {
  ...
  const builtinNames = builtinRole ? ((await loadBuiltinAgentDefinition(builtinRole))?.skills ?? []) : []
    ...
    return {
      hookSpecificOutput: {
        ...
        permissionDecisionReason:
          'Headless channel or scheduled turns cannot mutate agent configuration. Ask the user to make this change in Windbot Studio.'
      }
    }
  }
    ...
    if (interactionState.currentTurn === 'headless' || interactionState.userResponse === 'unavailable') {
      return {
        hookSpecificOutput: {
          ...
          permissionDecision: 'deny',
          permissionDecisionReason:
            'Headless channel or scheduled turns cannot submit Windbot Studio feedback. Keep the local feedback draft for an interactive user to review and submit.'
        }
      }
    }
    return {
      hookSpecificOutput: {
        hookEventName: 'PreToolUse',
        permissionDecision: 'ask',
        permissionDecisionReason: 'Submitting Windbot Studio feedback to Feishu requires live per-call user approval.'
      }
    }
  }
...
export async function buildSystemPrompt(
  ...
  // Runtime and tool-selection strategy lives in the default-enabled windbot-tool-guide skill.
