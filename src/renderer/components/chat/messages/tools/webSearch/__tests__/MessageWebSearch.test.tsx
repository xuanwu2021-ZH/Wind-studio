...
describe('MessageWebSearchToolTitle', () => {
  it('shows the query and an empty-result label without a disclosure', () => {
    ...
    expect(screen.getByText('Wind Studio')).toBeInTheDocument()
  ...
  it('shows the query in the header and renders each result as a link with favicon and domain', async () => {
        ...
        toolResponse={
          {
            ...
            arguments: { query: 'Wind Studio' },
            response: [
              { id: 1, title: 'Wind Studio', url: 'https://www.windbot.cn/blog', content: 'Wind Studio' }
            ]
          } as NormalToolResponse
        }
      />
    ...
    expect(within(header).getByText('Wind Studio')).toBeInTheDocument()
