...
## 6. Target vs Current State

The top-level decomposition is current: `src/shared/` contains only `ai/`, `data/`, `ipc/`, `types/`, and `utils/`. The table below records the remaining known gaps rather than the history of completed moves.
...
| `IpcChannel.ts` | v1 channel enum at the root, still used by legacy domains and the data/IpcApi transport channels | retire legacy entries domain by domain, then move the remaining infrastructure channels under `ipc/` |
