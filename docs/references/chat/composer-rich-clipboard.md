...
tokens when users copy and paste between Wind Studio message surfaces and the
composer.
...
- Keep regular clipboard behavior useful outside Wind Studio through
...
| `web application/x-cherry-composer-fragment+json` | Private Wind Studio token fragment. |
...
copies lose token identity after an app restart or in another Wind Studio
